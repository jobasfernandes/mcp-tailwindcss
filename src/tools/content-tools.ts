...
export function registerContentTools(mcpServer: McpServer, srcPath: string) {
  // Tool: tailwind_integracoes
  mcpServer.registerTool(
    'tailwind_integracoes',
    {
      ...
      inputSchema: {
        framework: z.string().describe('Framework alvo (vite, next, nuxt, sveltekit, remix, astro, vue, react).'),
      },
...
@tailwind utilities;`,
      ...
      if (!guide) {
        ...
        return {
          content: [{ type: 'text' as const, text: `❌ Framework não suportado: ${framework}\n\nFrameworks disponíveis: ${available}` }],
          isError: true,
        }
      }
  ...
  mcpServer.registerTool(
    'tailwind_utilities',
    {
      ...
      inputSchema: {
        categoria: z.enum(['layout', 'flexbox', 'grid', 'spacing', 'sizing', 'typography', 'backgrounds', 'borders', 'effects', 'filters', 'transitions', 'transforms', 'interactivity', 'svg', 'tables', 'accessibility']).optional().describe('Categoria de utilities para listar. Se omitido, lista todas as categorias disponíveis.'),
      },
      annotations: { readOnlyHint: true },
    },
    async ({ categoria }) => {
      ...
      if (!cat) {
        return {
          content: [{ type: 'text' as const, text: `❌ Categoria não encontrada: ${categoria}\n\nCategorias disponíveis: ${Object.keys(utilities).join(', ')}` }],
          isError: true,
        }
      }
  ...
  mcpServer.registerTool(
    'tailwind_variants',
    {
      ...
      inputSchema: {
        tipo: z.enum(['pseudo-classes', 'pseudo-elements', 'responsive', 'dark-mode', 'state', 'compound', 'aria', 'data']).optional().describe('Tipo de variant para listar. Se omitido, lista todos os tipos.'),
      },
      annotations: { readOnlyHint: true },
    },
    async ({ tipo }) => {
      ...
      if (!varType) {
        return {
          content: [{ type: 'text' as const, text: `❌ Tipo não encontrado: ${tipo}\n\nTipos disponíveis: ${Object.keys(variants).join(', ')}` }],
          isError: true,
        }
      }
  ...
  mcpServer.registerTool(
    'tailwind_cores',
    {
      ...
      inputSchema: {
        cor: z.string().optional().describe('Nome da cor específica (slate, gray, zinc, neutral, stone, red, orange, amber, yellow, lime, green, emerald, teal, cyan, sky, blue, indigo, violet, purple, fuchsia, pink, rose). Se omitido, lista todas.'),
      },
      annotations: { readOnlyHint: true },
    },
    async ({ cor: corArg }) => {
      ...
      if (!colorShades) {
        return {
          content: [{ type: 'text' as const, text: `❌ Cor não encontrada: ${corArg}\n\nCores disponíveis: ${Object.keys(colors).join(', ')}` }],
          isError: true,
        }
      }
  ...
  mcpServer.registerTool(
    'tailwind_receitas',
    {
      ...
      inputSchema: {
        componente: z.enum(['button', 'card', 'form', 'input', 'modal', 'navbar', 'footer', 'hero', 'grid', 'alert', 'badge', 'avatar', 'dropdown', 'tabs', 'breadcrumb', 'pagination', 'skeleton']).describe('Componente específico para ver a receita.'),
      },
      annotations: { readOnlyHint: true },
    },
    async ({ componente }) => {
...
<div class="animate-pulse space-y-4">
  ...
  <div class="flex items-center gap-4">
    ...
    <div class="flex-1 space-y-2">
      ...
      if (!componente) {
        return {
          content: [{ type: 'text' as const, text: `❌ Parâmetro 'componente' é obrigatório.\n\nComponentes disponíveis: ${Object.keys(receitas).join(', ')}` }],
          isError: true,
        }
      }
      ...
      if (!receita) {
        return {
          content: [{ type: 'text' as const, text: `❌ Componente não encontrado: ${componente}\n\nComponentes disponíveis: ${Object.keys(receitas).join(', ')}` }],
          isError: true,
        }
      }
  ...
  // Tool: tailwind_migracao_v4
  mcpServer.registerTool(
    'tailwind_migracao_v4',
    {
      ...
      inputSchema: {
        topico: z.enum(['overview', 'css-first', 'theme-config', 'utilities', 'variants', 'plugins', 'breaking-changes', 'postcss-vite']).optional().describe('Tópico específico da migração. Se omitido, mostra overview completo.'),
      },
      annotations: { readOnlyHint: true },
    },
    async ({ topico }) => {
...
\`\`\``,
      ...
      if (!content) {
        return {
          content: [{ type: 'text' as const, text: `❌ Tópico não encontrado: ${topico}\n\nTópicos disponíveis: ${Object.keys(migracaoContent).join(', ')}` }],
          isError: true,
        }
      }
  ...
  // Tool: tailwind_boas_praticas
  mcpServer.registerTool(
    'tailwind_boas_praticas',
    {
      ...
      inputSchema: {
        topico: z.enum(['organizacao', 'componentes', 'performance', 'acessibilidade', 'responsivo', 'dark-mode', 'animacoes', 'forms']).optional().describe('Tópico específico. Se omitido, mostra dicas gerais.'),
      },
      annotations: { readOnlyHint: true },
    },
    async ({ topico }) => {
